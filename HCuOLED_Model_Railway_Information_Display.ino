/* FILE:    HCuOLED_Model_Railway_Infomation_Display
   DATE:    22/01/22
   VERSION: 0.1
   AUTHOR: Chris Sharp
...
byte m=27;    // the minute value the clock will start at
byte s=40;    // the second value the clock will start at
...
/* create the train depatures for each train: Minutes, Platform, Destination */
...
void displayClock()
{
  byte col = 98;                      // position of left edge of the clock from left side of screen in pixels
  byte row = 58;                      // position of top edge of teh clock from the top of screen in pxels
  HCuOLED.SetFont(sharpsharp_6pt);    // set font for hours and minutes
  ...
  } else if (h==0)                    // a value of 0 will leave a blank space so check for it
  {
    ...
    HCuOLED.Print("0");               // when hours is a single digit value print a leading zero
    HCuOLED.Cursor(col+4, row);       // move cursor 4 pixels across to clear the leading zero
    HCuOLED.Print(h);
  }
  HCuOLED.Cursor(col+10, row);
  HCuOLED.Print(":");                 // print a colon between hours and minutes
  ...
  /* display minutes */
  HCuOLED.SetFont(sharpsharp_5pt);  // set smaller font for seconds
...
{ 
  ...
  /* loop through all the departures to generate the display */
  for (int i = 0; i < NUMBER_OF_DEPARTURES; i++)
  {
    displayDeparture(i);
  }
